...
async function fetchMachineData() {
    try {
        const response = await fetch(`${API_BASE}/dryer-machines`);
...
async function showMachineModal(filterType) {
    try {
        ...
        const titles = {
            'all': 'All Dryer Machines',
            'available': 'Available Dryers',
            'in-use': 'Dryers In Use',
            'faulty': 'Faulty Dryers'
        };
...
function generateModalMachineCard(machine) {
    ...
    return `
        <div class="modal-machine-item ${statusClass}">
            <div class="machine-num">D${machine.machineNumber}</div>
...
function setupBookingFormHandlers() {
    ...
    const dryTypeBtns = document.querySelectorAll('.wash-type-btn');
    dryTypeBtns.forEach(btn => {
        btn.addEventListener('click', () => {
            dryTypeBtns.forEach(b => b.classList.remove('selected'));
            btn.classList.add('selected');
            selectedDryType = btn.dataset.type;
...
async function loadAvailableMachines() {
    try {
        ...
        availableMachines.forEach(machine => {
            ...
            option.textContent = `Dryer ${machine.machineNumber}`;
...
function updateBookingSummary() {
    ...
    if (selectedDryType && selectedTimeSlot && selectedMachine) {
        ...
        document.getElementById('summaryMachine').textContent = `Dryer ${selectedMachine}`;
        document.getElementById('summaryType').textContent = selectedDryType.charAt(0).toUpperCase() + selectedDryType.slice(1) + ' Dry';
...
async function handleBookingConfirm() {
    ...
    try {
        ...
        const response = await fetch(`${API_BASE}/dryer-machines/book`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                machineNumber: parseInt(selectedMachine),
                dryType: selectedDryType,
        ...
        if (response.ok && result.success) {
            alert(`Booking successful! Dryer ${selectedMachine} is reserved for ${selectedDryType} dry at ${selectedTimeSlot.display}.`);
    ...
    if (notificationTime > 0) {
        setTimeout(() => {
            if (Notification.permission === 'granted') {
                new Notification('ZenCampus - Dryer Machine', {
                    body: `Dryer ${machineNumber} will be free in 5 minutes!`,
                    icon: '/favicon.ico'
                });
            } else {
                alert(`Dryer ${machineNumber} will be free in 5 minutes!`);
            }
        }, notificationTime);
    }
