...
export const GET = requireAdmin(async (req: NextRequest) => {
  try {
    ...
    const maintenanceByType = {
      electrical: {
        ...
        completed: complaints.filter(
          (c) => c.type === "Electrical" && c.status === "Completed"
        ).length,
      ...
      plumbing: {
        ...
        completed: complaints.filter(
          (c) => c.type === "Plumbing" && c.status === "Completed"
        ).length,
        pending: complaints.filter(
          (c) => c.type === "Plumbing" && c.status !== "Completed"
        ).length,
      ...
      carpentry: {
        ...
        completed: complaints.filter(
          (c) => c.type === "Carpentry" && c.status === "Completed"
        ).length,
        pending: complaints.filter(
          (c) => c.type === "Carpentry" && c.status !== "Completed"
        ).length,
      ...
      general: {
        ...
        completed: complaints.filter(
          (c) => c.type === "General" && c.status === "Completed"
        ).length,
        pending: complaints.filter(
          (c) => c.type === "General" && c.status !== "Completed"
        ).length,
    ...
    const laundryStats = {
      washingMachine: laundryBookings.filter(
        (b) => b.machineType === "Washing Machine"
      ).length,
      dryer: laundryBookings.filter((b) => b.machineType === "Dryer").length,
    };

    const canteenRevenue = canteenOrders.reduce(
    ...
    const cleaningStats = {
      roomCleaning: {
        completed: cleaningRequests.filter(
          (r) => r.type === "Room" && r.status === "Completed"
        ).length,
        pending: cleaningRequests.filter(
          (r) => r.type === "Room" && r.status !== "Completed"
        ).length,
      },
      bathroomCleaning: {
        completed: cleaningRequests.filter(
          (r) => r.type === "Bathroom" && r.status === "Completed"
        ).length,
        pending: cleaningRequests.filter(
          (r) => r.type === "Bathroom" && r.status !== "Completed"
        ).length,
      ...
      commonArea: {
        completed: cleaningRequests.filter(
          (r) => r.type === "Common Area" && r.status === "Completed"
        ).length,
        pending: cleaningRequests.filter(
          (r) => r.type === "Common Area" && r.status !== "Completed"
        ).length,
